...
final class AudioCapture: @unchecked Sendable {
  ...
  func start(
    format: AVAudioFormat, yield: @escaping @Sendable (AnalyzerInput) -> Void,
    ...
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
      queue.async {
        do {
          ...
          else { throw PilotError.unavailable("No usable microphone. Check Sound settings.") }
          ...
          input.installTap(onBus: 0, bufferSize: 2048, format: native) { buffer, _ in
            ...
            guard let copy = AVAudioPCMBuffer(pcmFormat: native, frameCapacity: buffer.frameLength)
            else { return }
            copy.frameLength = buffer.frameLength
            let source = UnsafeMutableAudioBufferListPointer(buffer.mutableAudioBufferList)
            let target = UnsafeMutableAudioBufferListPointer(copy.mutableAudioBufferList)
            for (a, b) in zip(source, target) {
              if let from = a.mData, let to = b.mData { memcpy(to, from, Int(a.mDataByteSize)) }
            }
            self.queue.async {
              guard self.engine != nil else { return }
              let capacity =
                AVAudioFrameCount(Double(copy.frameLength) * format.sampleRate / native.sampleRate)
                + 32
              ...
              if Date().timeIntervalSince(lastLevel) > 0.08, let data = copy.floatChannelData?[0] {
                ...
                level(min(1, sqrt(sum / Float(max(1, copy.frameLength))) * 12))
                lastLevel = Date()
              }
            }
          }
