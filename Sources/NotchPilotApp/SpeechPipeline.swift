...
actor SpeechPipeline {
  ...
  func prepare(language: String, installAssets: Bool = false) async throws {
    ...
    else { throw PilotError.unavailable("This speech language is not available on this Mac.") }
    ...
    if status != .installed {
      guard installAssets else {
        throw PilotError.unavailable("Install on-device speech in Settings before listening.")
      }
    ...
    else { throw PilotError.unavailable("No compatible on-device audio format.") }
  ...
  func start(
    language: String, update: @escaping @Sendable (SpeechUpdate) -> Void,
    level: @escaping @Sendable (Float) -> Void, failure: @escaping @Sendable (String) -> Void
  ) async throws {
    let session = UUID()
    generation = session
    try await prepare(language: language)
    try Task.checkCancellation()
    guard generation == session, let transcriber, let analyzer, let format else {
    ...
    resultsTask = Task {
      ...
      do {
        for try await result in transcriber.results {
          ...
          update(
            SpeechUpdate(
              text: result.isFinal ? committed : committed + segment, committed: committed,
              isFinal: result.isFinal))
        }
      } catch is CancellationError {} catch {
        failure("On-device recognition stopped. Start a new session.")
      }
    }
    ...
    if generation != session || Task.isCancelled {
      await capture.stop()
      throw CancellationError()
    }
  }
  ...
  func transcribeFixture(
    _ url: URL, language: String, update: @escaping @Sendable (SpeechUpdate) -> Void
  ) async throws {
    try await prepare(language: language)
    ...
    guard let format = self.format,
      ...
      throw PilotError.unavailable("Unsupported fixture format")
    }
    let (stream, continuation) = AsyncStream<AnalyzerInput>.makeStream()
    try await analyzer.start(inputSequence: stream)
    while file.framePosition < file.length {
      try Task.checkCancellation()
      guard let buffer = AVAudioPCMBuffer(pcmFormat: file.processingFormat, frameCapacity: 2048)
      else { break }
      try file.read(into: buffer)
      let capacity =
        AVAudioFrameCount(
          Double(buffer.frameLength) * format.sampleRate / file.processingFormat.sampleRate) + 32
      ...
      try await Task.sleep(
        nanoseconds: UInt64(Double(buffer.frameLength) / file.processingFormat.sampleRate * 1e9))
    }
    continuation.finish()
