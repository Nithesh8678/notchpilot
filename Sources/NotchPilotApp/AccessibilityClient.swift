...
final class AccessibilityClient: @unchecked Sendable {
  ...
  init(pid: pid_t) throws {
    guard AXIsProcessTrusted() else {
      throw PilotError.unavailable(
        "Allow NotchPilot in System Settings → Privacy & Security → Accessibility.")
    }
    self.pid = pid
    ...
    if AXObserverCreate(
      ...
      { _, _, _, ref in
        if let ref {
          Unmanaged<AccessibilityClient>.fromOpaque(ref).takeUnretainedValue().signal.signal()
        }
      }, &observer) == .success, let observer
    {
      ...
      for name in [
        ...
        kAXValueChangedNotification, kAXLayoutChangedNotification,
      ] {
        AXObserverAddNotification(
          observer, app, name as CFString, Unmanaged.passUnretained(self).toOpaque())
      }
  ...
  func node(_ element: AXUIElement) -> AXNode {
    AXNode(
      element: element, role: string(element, kAXRoleAttribute),
      title: string(element, kAXTitleAttribute), label: string(element, kAXDescriptionAttribute),
  ...
  func unique(identifier: String, root: AXUIElement? = nil) throws -> AXNode {
    ...
    guard matches.count == 1 else {
      throw PilotError.unavailable(
        "WhatsApp's interface changed or is not ready. No action was taken.")
    }
    return matches[0]
  }
  func wait(
    token: CancellationToken, revision: Int? = nil, timeout: TimeInterval = 10,
  ...
  func press(_ node: AXNode, token: CancellationToken, revision: Int) throws {
    ...
    guard AXUIElementPerformAction(node.element, kAXPressAction as CFString) == .success else {
      throw PilotError.unavailable("This control does not expose a supported press action.")
    }
  }
  func setText(
    ...
    guard old.isEmpty || old == previous || old == text else {
    ...
    for start in stride(from: 0, to: units.count, by: 16) {
      ...
      else { throw PilotError.unavailable("Keyboard input unavailable.") }
  ...
  func key(_ code: CGKeyCode, flags: CGEventFlags = [], token: CancellationToken, revision: Int)
    throws
  {
    try token.check(revision: revision)
    guard NSWorkspace.shared.frontmostApplication?.processIdentifier == pid else {
