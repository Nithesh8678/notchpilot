...
@MainActor final class HotkeyManager {
    ...
    init() {
        ...
        InstallEventHandler(GetApplicationEventTarget(), { _, event, data in
            ...
            MainActor.assumeIsolated {
                if id.id == 2 { if pressed { manager.onCancel?() } }
                else if pressed { manager.onPress?() } else { manager.onRelease?() }
            }
            return noErr
    ...
    func register(code: UInt32, modifiers: UInt32) -> Bool {
        ...
        return RegisterEventHotKey(code, modifiers, EventHotKeyID(signature: 0x4E504C54, id: 1), GetApplicationEventTarget(), 0, &hotkey) == noErr
    }
    func enableEscape(_ enabled: Bool) {
        if let escape { UnregisterEventHotKey(escape) }; escape = nil
        if enabled { RegisterEventHotKey(53, 0, EventHotKeyID(signature: 0x4E504C54, id: 2), GetApplicationEventTarget(), 0, &escape) }
    }
}
