...
public struct CommandBatch: Sendable {
  ...
  public init(revision: Int, commands: [Command], stableEnd: Int, committedEnd: Int) {
    self.revision = revision
    self.commands = commands
    self.stableEnd = stableEnd
...
public struct CommandStateMachine: Sendable {
  ...
  public mutating func ingest(text: String, committed: String, now: TimeInterval) -> CommandBatch {
    ...
    if commands.contains(where: { $0.kind == .cancel && $0.endOffset <= stable }) {
      cancelled = true
    }
    return CommandBatch(
      revision: revision, commands: cancelled ? [] : commands, stableEnd: stable,
      committedEnd: (committed as NSString).length)
  }
...
public final class CancellationToken: @unchecked Sendable {
  ...
  public init() {}
  public func update(revision: Int) {
    lock.lock()
    self.revision = revision
    lock.unlock()
  }
...
public struct SafetyPolicy: Sendable {
  ...
  public init(dryRun: Bool = true, confirmAll: Bool = false) {
    self.dryRun = dryRun
  ...
  public func needsConfirmation(_ message: String) -> Bool {
    ...
    let risks = [
      "password", "passcode", "otp", "verification code", "bank", "account number", "transfer",
      "pay", "payment", "purchase", "buy", "delete", "security", "passport", "aadhaar", "identity",
      "credit card", "pin", "public", "post", "money",
    ]
...
public struct ExecutionEvent: Sendable {
  ...
  public init(kind: CommandKind, status: String, milliseconds: Double) {
    self.kind = kind
    self.status = status
...
public actor ActionQueue {
  ...
  public init(
    ...
    event: @escaping @Sendable (ExecutionEvent) -> Void
  ) {
  ...
  public func submit(_ batch: CommandBatch) {
    ...
    for old in completed.values where old.kind != .typeText {
      if batch.commands.first(where: { $0.id == old.id }) != old {
        ...
        event(
          .init(
            kind: old.kind, status: "error: Speech changed an executed command. Start again.",
            milliseconds: 0))
        return
      }
    }
      ...
      do {
        ...
        if command.kind == .send {
          ...
          guard !message.isEmpty else {
            throw PilotError.unsafe("Dictate a message before sending.")
          }
          if !policy.dryRun && policy.needsConfirmation(message)
            && approvedRevision != currentRevision
          {
            pending.insert(command, at: 0)
            event(.init(kind: .send, status: "confirmation", milliseconds: 0))
            return
          }
        }
        ...
        let result = try await adapter.execute(
          command, token: token, revision: currentRevision, policy: policy)
