...
public struct DisplayLayout: Sendable {
    ...
    public init(frame: CGRect, visibleFrame: CGRect, topInset: CGFloat = 0, leftAuxiliary: CGRect? = nil, rightAuxiliary: CGRect? = nil) {
        self.frame = frame; self.visibleFrame = visibleFrame; self.topInset = topInset
        self.leftAuxiliary = leftAuxiliary; self.rightAuxiliary = rightAuxiliary
    }
    public var housing: CGRect? {
        guard topInset > 0, let l = leftAuxiliary, let r = rightAuxiliary, r.minX > l.maxX else { return nil }
    ...
    public func capsule(expanded: Bool) -> CGRect {
        ...
        return CGRect(x: (housing?.midX ?? frame.midX) - width / 2, y: top - height, width: width, height: height)
    }
}
public enum OverlayState: String, Sendable { case idle, activating, listening, understanding, executing, success, needsConfirmation, cancelled, error }
...
public struct PermissionSnapshot: Sendable {
    ...
    public init(microphone: PermissionState, accessibility: Bool) { self.microphone = microphone; self.accessibility = accessibility }
}
